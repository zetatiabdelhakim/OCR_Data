"""Image-heavy A4 layout: photo galleries, floating-image paragraphs, the occasional table."""

import random
from core import assets, components, hybrid

NAME = "a4_image_gallery"
BASE_MM = (210, 297)
...
def generate(hybrid_html=""):
    ...
    for r in range(num_rows):
        ...
        rows_html += f'<div style="display: grid; gap: 16px; grid-template-columns: {col_layout}; height: 100%;">'
        placed_hybrid = False
        for _ in range(len(cols)):
            if r == 0:
                ...
                comp_type = random.choices(
                    ["gallery", "complex_para", "table", "figure"], weights=[0.35, 0.35, 0.1, 0.2]
                )[0]
                if comp_type == "gallery":
                    comp = components.gen_image_gallery(n=random.choice([2, 3, 4]))
                elif comp_type == "complex_para":
                    comp = components.gen_complex_paragraph()
                elif comp_type == "table":
                    comp = components.gen_table(rows=random.randint(4, 8), cols=random.randint(2, 3))
                else:
                    comp = components.gen_figure()
            rows_html += f'<div style="height: 100%; overflow: hidden; display: flex; flex-direction: column;">{comp}</div>'
        rows_html += "</div>"

    body = (f'<div id="page-grid" style="width:100%; height:100%; padding:36px; box-sizing:border-box; '
            f'display: grid; gap: 16px; grid-template-rows: {" ".join(row_heights)};">{rows_html}</div>')
