...
def generate(hybrid_html=""):
    ...
    letterhead = assets.get_semantic_org()
    city = assets.get_semantic_city()
    date_line = f"{city}، في {random.randint(1,28):02d}/{random.randint(1,12):02d}/{random.randint(2023,2026)}"
    recipient = random.choice(assets.LETTER_OPENERS)
    # Subject uses a real section/topic heading rather than corpus noise
    subject = "الموضوع: " + assets.get_semantic_subject()
    ...
    signature_name = assets.get_semantic_name()
    ...
    blocks = [
        block("letterhead", letterhead, size=20, bold=True, align="center", use_title_font=True),
    ...
    for p in body_paras:
        blocks.append(block("body-paragraph", p, size=14, align="right"))
